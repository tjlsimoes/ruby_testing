...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        allow(game_input).to receive(:gets).and_return("a", "1")
      end
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        allow(game_input).to receive(:gets).and_return("a", "11" , "1")
      end
  ...
  describe '#verify_input' do
    ...
    subject(:game_v_input) { described_class.new(1, 10) }

    context 'when given a valid input as argument' do
      it 'returns valid input' do
        received_value = 4
        min = game_v_input.instance_variable_get(:@minimum)
        max = game_v_input.instance_variable_get(:@maximum)
        v_input = game_v_input.verify_input(min, max, received_value)

        expect(v_input).to eq(received_value)
  ...
  describe '#display_turn_order' do
    ...
    subject(:game_turn) { described_class.new(2, 20) }
    ...
    it 'increases guess_count by one' do
      expect { game_turn.display_turn_order(search_turn) }.to change {
        game_turn.instance_variable_get(:@guess_count)}.by(1)
    end
