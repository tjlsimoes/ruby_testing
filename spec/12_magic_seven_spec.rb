...
describe MagicSeven do
  ...
  describe '#play' do
    context 'when the random number is 18' do
      ...
      subject(:game_eighteen) { described_class.new(18) }

      it 'will return 7' do
        result = game_eighteen.play
        expect(result).to eq(7)
      end
    end
    ...
    context 'when the random number is 21' do
      # remove the 'x' before running this test
      subject(:game_twentyone) { described_class.new(21) }
      it 'will return 7' do
        result = game_twentyone.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is 333' do
      # remove the 'x' before running this test
      subject(:game_grand) { described_class.new(333) }

      it 'will return 7' do
        result = game_grand.play
        expect(result).to eq(7)
      end
    end
  end
