...
describe FindNumber do
  ...
  describe '#game_over?' do
    ...
    let(:number_end) { double('random_number', value: 7) }
    ...
    context 'when guess and random_number are equal' do
      ...
      subject(:game_over) { described_class.new(0, 9, number_end, 7) }
    ...
    context 'when guess and random_number are not equal' do
      ...
      subject(:game_not_over) { described_class.new(0, 10, number_end, 5) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'updates min to 5' do
        guess = low_guess_game.guess
        low_guess_game.update_range
        min = low_guess_game.min
        expect(min).to eq(guess + 1)
      end

      it 'does not update max' do
        max_orig = low_guess_game.max
        low_guess_game.update_range
        max_new = low_guess_game.max
        expect(max_new).to eq(max_orig)
      end
    end
    ...
    context 'when the guess is more than the answer' do
      ...
      it 'does not update min' do
        min_orig = high_guess_game.min
        high_guess_game.update_range
        min_new = high_guess_game.min
        expect(min_new).to eq(min_orig)
      end

      it 'updates max to 8' do
        guess = high_guess_game.guess
        high_guess_game.update_range
        max = high_guess_game.max
        expect(max).to eq(guess - 1)
      end
    end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      subject(:narrow_guess_game) { described_class.new(5, 8, number_range, 7) }

      it 'updates min to the same value as max' do
        max = narrow_guess_game.max
        narrow_guess_game.update_range
        min = narrow_guess_game.min
        expect(min).to eq(max)
      end

      it 'does not update max' do
        max_orig = narrow_guess_game.max
        narrow_guess_game.update_range
        max_new = narrow_guess_game.max
        expect(max_new).to eq(max_orig)
      end
    end
